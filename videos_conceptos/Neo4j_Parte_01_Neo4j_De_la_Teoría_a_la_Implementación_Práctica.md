¿Alguna vez te has enfrentado a una consulta SQL con tantos `JOIN`s que se vuelve casi ilegible y terriblemente lenta? Este es un problema común con datos altamente conectados. Vamos a explorar cómo un cambio de paradigma, centrado en las relaciones, puede resolver esta 'tiranía del JOIN' de una vez por todas.
